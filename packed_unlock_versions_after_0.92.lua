-- Next line ensures the right game version is used. E.g. the position for globals change every game build, so it is advisable to add this always.
--require_game_build(2545)--游戏版本检测 无需检测全版本通用
----------------------------------packed bool----------------------------------
local packed_bool_unlock_menu = menu.add_submenu("Packed bool 解锁")--创建子菜单
...
packed_bool_unlock_menu:add_toggle("---------------解锁/锁定---------------", function()---解锁/锁定切换功能
	return unlock_state
	end,function()
...
packed_bool_unlock_menu:add_action("FIXERTATTOOSTAT 解锁联系人DLC物品",function()

	fixertattoostat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("FIXERPSTAT_BOOL 解锁联系人DLC物品",function()--联系人dlc
	...
	fixerpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("TUNERPSTAT_BOOL 解锁车友会DLC物品",function()

	tunerpstal_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("HISLANDPSTAT_BOOL 解锁佩里克岛DLC物品",function() --佩里克岛

	hislandpstal_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("SU20TATTOOSTAT_BOOL 解锁夏季更新DLC物品",function()--夏季更新
	...
	su20tattoostat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("SU20PSTAT_BOOL夏季更新",function()--夏季更新
	...
	su20pstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("HEIST3TATTOOSTAT_BOOL 赌场抢劫DLC物品",function()--赌场抢劫

	heist3tattoostat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("CASINOHSTPSTAT_BOOL 赌场抢劫DLC物品",function()--dc抢劫

	casinohstpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("CASINOPSTAT_BOOL 钻石赌场DLC物品",function() --钻石赌场

	casinopstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("ARENAWARSPSTAT_BOOL 竞技场DLC物品",function() --竞技场

	arenawarspstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("BUSINESSBATPSTAT_BOOL 夜总会DLC物品",function()--夜总会

	businessbatpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("GANGOPSPSTAT_BOOL 末日豪劫",function()---末日豪劫
	...
	gangopspstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("DLCSMUGCHARPSTAT 军火走私",function()--GTA Online： Smuggler's Run

	dlcsmugcharpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("GUNTATPSTAT_BOOL 军火走私",function()

	guntatpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("DLCGUNPSTAT_BOOL",function()--进出口大亨

	dlcgunpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("DLCBIKEPSTAT_BOOL",function()--狂野飙客
	
	dlcbikepstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("NGDLCPSTAT_BOOL",function()

	ngdlcpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("MP_NGDLCPSTAT_BOOL",function()

	mp_ngdlcpstat_bool_unlock(unlock_state)
...
packed_bool_unlock_menu:add_action("NGTATPSTAT_BOOL",function()

	ngtatpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("MP_NGPSTAT_BOOL",function()

	mp_ngpstat_bool_unlock(unlock_state)
...
packed_bool_unlock_menu:add_action("NGPSTAT_BOOL",function()

	ngpstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("TUPSTAT_BOOL",function()

	tupstat_bool_unlock(unlock_state,mpx)
...
packed_bool_unlock_menu:add_action("MP_TUPSTAT_BOOL",function()

	mp_tupstat_bool_unlock(unlock_state)
...
----------------------------------packed int----------------------------------
local packed_int_unlock_menu = menu.add_submenu("Packed int 解锁")--创建子菜单
...
packed_int_unlock_menu:add_action("解锁双动左轮 需要换占据再次使用",function()
...
packed_int_unlock_menu:add_action("解锁石斧 需要换占据再次使用",function()
